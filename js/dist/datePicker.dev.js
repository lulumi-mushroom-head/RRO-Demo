...
var displayBoxes = $(".date-picker-display"); // sensible default just in case jQuery doesn't kick in
...
var windowWidth = 300;
var colourPickerWidth = 300; // set up dates

$(document).ready(function () {
  // will work the same the first time as every other
  updateDatePicker(); // update dates shown to correct dates
...
$(document).ready(function () {
  ...
  $(".date-picker-date").click(function (e) {
    ...
    $(this).addClass("active"); // update currentDate
...
function updateDatePicker() {
  var changeMonth = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  ...
  updatePickerMonth(); // clear out all tr instances other than the header row
  ...
  var closeRow = "</tr>";
  var currentRow = openRow; // Add in as many of last month as required

  if (lastMonthToInclude > 0) {
    var lastMonthLastDay = addDays(firstOfMonth, -1);
    var lastMonthDays = lastMonthLastDay.getDate();
    var lastMonthStartAdding = lastMonthDays - lastMonthToInclude + 1; // add days from previous month
  ...
  addToCalendar(1, 7 - lastMonthToInclude, lastMonthToInclude, true); // reset for current month generation

  currentRow = openRow;
  var counter = 7;
  var addedFromCurrentMonth = 7 - firstWeekday + 1;
  addToCalendar(addedFromCurrentMonth, lastOfMonth, counter, true); // at this point, counter = all of this month + whatever was included from last month

  counter = lastMonthToInclude + lastOfMonth;
  var nextMonthToInclude = counter % 7 === 0 ? 0 : 7 - counter % 7;
  addToCalendar(1, nextMonthToInclude, counter, "next-month"); // add event listener again

  applyDateEventListener(); // update current date box

  updateDateShown(); // functions scoped to this outer function
  //############################################################

  function checkChangedMonth() {
    // updates if changed month directly (changeMonth) or if switched to other .date-picker-date and month is different (differentMonth)
    var differentMonth = false; // checks if it's the same date again
  ...
  function addToCalendar(start, end, counter, cellClass) {
    ...
    for (var i = start; i <= end; i++) {
      ...
      if (i === todayDate && currentMonth) {
        currentRow += "<td class=\"active\">".concat(i, "</td>");
      } else if (cellClass && !currentMonth) {
        currentRow += "<td class=\"".concat(cellClass, "\">").concat(i, "</td>");
      } else {
        currentRow += "<td>".concat(i, "</td>");
      }
...
function dateSelected(currentDay) {
  ...
  var activeDate = $($(".date-picker-date.active")[0]); // get current date and update
...
function changeMonth(direction) {
  var increment = direction === "Next" ? 1 : -1; // change month

  dates[currentDate] = addMonths(dates[currentDate], increment); // change month name in picker

  updatePickerMonth(); // update calendar
...
function showDatePicker(e) {
  ...
  datePicker.css("top", pxFromTop + 40); // check if right edge of colourPicker will go off the edge of the screen, and if so then reduce left by that amount
...
function applyDateEventListener() {
  $("#date-picker td").click(function () {
    ...
    $(this).addClass("active"); // update variables

    currentDay = $(this).text(); // update the current date

    dateSelected(currentDay); // change month based on calendar day class
